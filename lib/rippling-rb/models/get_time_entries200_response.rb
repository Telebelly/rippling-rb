...
module RipplingRb
  class GetTimeEntries200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RipplingRb::GetTimeEntries200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RipplingRb::GetTimeEntries200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [__meta, id, created_at, updated_at, worker_id, worker, start_time, end_time, comments, job_shifts, breaks, premiums, piece_rate_premiums, segments, time_entry_summary, time_card_id, time_card, tags, idempotency_key, create_extra_hours_run, status, pay_period, shift_input_values].hash
    end
