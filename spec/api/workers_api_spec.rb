=begin
#Rippling Platform API

#Documentation for the Rippling Platform API.

The version of the OpenAPI document: 1.0.0

Generated by: https://openapi-generator.tech
Generator version: 7.14.0

=end

require 'spec_helper'
require 'json'

# Unit tests for OpenapiClient::WorkersApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'WorkersApi' do
  before do
    # run before each test
    @api_instance = OpenapiClient::WorkersApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of WorkersApi' do
    it 'should create an instance of WorkersApi' do
      expect(@api_instance).to be_instance_of(OpenapiClient::WorkersApi)
    end
  end

  # unit tests for get_workers
  # Retrieve a specific worker
  # Retrieve a specific worker
  # @param id ID of the resource to return
  # @param [Hash] opts the optional parameters
  # @option opts [String] :expand 
  # @return [GetWorkers200Response]
  describe 'get_workers test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for list_workers
  # List workers
  # A List of workers  - Requires: &#x60;API Tier 1&#x60;  - Filterable fields: &#x60;status&#x60;, &#x60;work_email&#x60;, &#x60;user_id&#x60;  - Expandable fields: &#x60;user&#x60;, &#x60;manager&#x60;, &#x60;legal_entity&#x60;, &#x60;employment_type&#x60;, &#x60;compensation&#x60;, &#x60;department&#x60;, &#x60;teams&#x60;, &#x60;level&#x60;, &#x60;custom_fields&#x60;  - Sortable fields: &#x60;id&#x60;, &#x60;created_at&#x60;, &#x60;updated_at&#x60;
  # @param [Hash] opts the optional parameters
  # @option opts [String] :filter 
  # @option opts [String] :expand 
  # @option opts [String] :order_by 
  # @return [ListWorkers200Response]
  describe 'list_workers test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
