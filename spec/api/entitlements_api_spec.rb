=begin
#Rippling Platform API

#Documentation for the Rippling Platform API.

The version of the OpenAPI document: 1.0.0

Generated by: https://openapi-generator.tech
Generator version: 7.14.0

=end

require 'spec_helper'
require 'json'

# Unit tests for OpenapiClient::EntitlementsApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'EntitlementsApi' do
  before do
    # run before each test
    @api_instance = OpenapiClient::EntitlementsApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of EntitlementsApi' do
    it 'should create an instance of EntitlementsApi' do
      expect(@api_instance).to be_instance_of(OpenapiClient::EntitlementsApi)
    end
  end

  # unit tests for list_entitlements
  # List entitlements
  # A List of entitlements  - Requires: &#x60;API Tier 1&#x60;
  # @param [Hash] opts the optional parameters
  # @return [ListEntitlements200Response]
  describe 'list_entitlements test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
